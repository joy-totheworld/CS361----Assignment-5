...
selectionButton.addEventListener("click", function () { addSelections(); console.log("addedClasses: ", addedClasses) }, false)
...
submitButton.addEventListener("click", displaySubmitWarning)

const clearButton = document.getElementById("clear-button")
clearButton.addEventListener("click", displayClearWarning)
...
function removeSelection() {
    ...
    console.log('document.getElementsByClassName("term")[removalIdx]: ', document.getElementsByClassName("term")[removalIdx])
...
function addSelections() {
    ...
    var addedClassInput = document.getElementById('class-input');
    ...
    for (var i = 0; i < allClassOptions.length; i++) {
        if (allClassOptions[i].value == addedClassInput.value) {
    ...
    if ((typeof selectedClass == "undefined") || (typeof openTerm.selectedIndex == "undefined")) {
        ...
        classDiv.addEventListener("click", function () {
            ...
            detailsID.innerHTML = classDetailArray.courseID
            detailsNameHeader.innerHTML = "Class Name: "
            detailsName.innerHTML = classDetailArray.courseName
        ...
        selectedTermContainer.prepend(classDiv);
        ...
        fetch("/POSTTOSERVER", {
            ...
            body: JSON.stringify(newAddedClass)
        })
            .then(resp => {
                if (resp.status === 200) {
                    return resp.json()
...
function addTermSelections() {
    ...
    if ((typeof terms[0].getElementsByClassName("term-name")[0].value !== "undefined") && (typeof terms[0].getElementsByClassName("term-name")[1].value !== "undefined")) {
        var newTerm = document.createElement("option");
        names = terms[0].getElementsByClassName("term-name")
        // console.log(names)
        newTerm.value = names[0].value + " " + names[1].value
        newTerm.innerText = names[0].value + " " + names[1].value + " "
        terms[0].dataset.id = names[0].value + " " + names[1].value
    ...
    for (var j = 1; j < terms.length; j += 1) {
        ...
        if ((typeof terms[j].getElementsByClassName("term-name")[0].value !== "undefined") && (typeof terms[j].getElementsByClassName("term-name")[1].value !== "undefined")) {
            ...
            // console.log(names)
            var currTermString = names[0].value.substring(0, 4) + " - " + names[1].value.substring(2, 4) + " " + names[2].value
            newTerm.value = currTermString
            newTerm.innerText = currTermString
            terms[j].dataset.id = currTermString
            newTerm.classList.add("dropdown-term")
            // console.log(newTerm)
            var classContainer = document.getElementById('term-selection');
            classContainer.appendChild(newTerm);
...
function addClass(courseID, courseName, prereqs) {
    var newClass = new Class(courseID, courseName, prereqs)
    allClasses.push(newClass)
...
function displayClearWarning() {
    ...
    noticeHeader.textContent = "Warning: "
    var notice1 = document.createElement("p")
    notice1.textContent = "Once you clear this plan, you will no longer be able to recover it."
    var notice2 = document.createElement("p")
    notice2.textContent = "Would you still like to clear it?."
    var clear2 = document.createElement("button")
    clear2.textContent = "Clear"
    clear2.classList.add("confirm-clear")
    var cancelButton = document.getElementById("continue-button")
    cancelButton.textContent = "Cancel"
    var fill1 = document.createElement("p")
    var fill2 = document.createElement("p")
    ...
    notificationTextContainer.append(notice1)
    notificationTextContainer.append(notice2)
    notificationTextContainer.append(clear2)
    notificationTextContainer.append(fill1)
    notificationTextContainer.append(fill2)

    clear2.addEventListener("click", clearPlan)
